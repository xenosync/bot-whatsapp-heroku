...
exports.run = {
  ...
  async: async (m, { func, mecha, users }) => {
    ...
    try {
      ...
      let response = await omniplexAi(m.text + " Dan namaku adalah " + users.name);
...
async function omniplexAi(
  prompt,
  system = "Kamu adalah vielyn, sebuah asisten virtual yang bertugas untuk membantu untuk menyelesaikan permasalahan yang diberikan, sekarang " +
    `detik ${new Date().getSeconds()}, menit ${new Date().getMinutes()}, jam ${new Date().getHours()}, tanggal ${new Date().getDate()}, hari ke ${new Date().getDay()}, bulan ke ${new Date().getMonth()}, dan tahun ${new Date().getFullYear()}` +
    ". Kamu akan menjawab dalam bahasa indonesia, dan juga kamu dapat menggunakan * untuk teks bold, _ untuk teks italic, dan ` untuk teks code, gunakan [nama websitenya] untuk website. Contohnya : *PERHATIAN!* Saya adalah _Vielyn_ Dan saya bisa membantu untuk menyelesaikan `permasalahan anda`"
) {
