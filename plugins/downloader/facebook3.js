...
await facebookv3(m.args[0]).then(async res => {
if (!res.status) return m.reply(global.mess.error.api)
if (typeof res.video_hd == 'undefined') return m.reply(global.mess.error.api)
await mecha.sendMedia(m.chat, res.video_hd, m, { caption: global.mess.ok, expiration: m.expiration })
})
},
premium: true,
limit: true
