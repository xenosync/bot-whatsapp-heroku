...
async function instagram(url) {
  const apiURL = `https://api.alyachan.dev/api/ig?url=${url}&apikey=6nY0bL`;
  try {
    const response = await axios.get(apiURL);
    const data = response.data;
    if (!data || !data.status || data.data.length === 0) return { status: false, message: 'Tidak ada data yang ditemukan.' };
