...
caption += commands.sort((a, b) => a.usage.localeCompare(b.usage)).map(v => styles(`	◦  ${m.prefix + v.usage} ${v.use} ${v.info}`)).join('\n')
if (style === 1) {
...
caption += commands.sort((a, b) => a.usage.localeCompare(b.usage)).map(v => styles(`	◦  ${m.prefix + v.usage} ${v.use}`)).join('\n')
}
caption ;
if (style === 1) {
mecha.reply(m.chat, caption + '\n\n' + global.footer, m, {
...
var yStr = ['ᴀ', 'ʙ', 'ᴄ', 'ᴅ', 'ᴇ', 'ꜰ', 'ɢ', 'ʜ', 'ɪ', 'ᴊ', 'ᴋ', 'ʟ', 'ᴍ', 'ɴ', 'ᴏ', 'ᴘ', 'q', 'ʀ', 'ꜱ', 'ᴛ', 'ᴜ', 'ᴠ', 'ᴡ', 'x', 'ʏ', 'ᴢ', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0']
var replacer = []
